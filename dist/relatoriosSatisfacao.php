...
//print_r($_REQUEST); echo "aqui";exit();

$datainicial = date("Y-m-d");
// $dadosPesquisas = 0;

if (isset($_REQUEST['requestSatisfacao'])) {
    ...
    $questao = !empty($_REQUEST['inputQuestao']) ? "(fk_enunciado = " .  $_REQUEST['inputQuestao'] . ') AND' : "";
    ...
    //Listar as pesquis"as
    $query_pesquisas = "SELECT pk_reservas, fk_espacos, enunciado, secretaria, espaços.nome, hr_ini, hr_fim, fk_alternativa, motivo, 
                    DATE_FORMAT(dt_ini, '%d/%m/%Y') as dt_ini, 
                    DATE_FORMAT(dt_fim, '%d/%m/%Y') as dt_fim, contato 
                    FROM reservas inner join satisfacao on fk_reservas = pk_reservas 
                    inner join resposta on fk_satisfacao = pk_satisfacao 
                    inner join eventos on fk_eventos = pk_eventos inner join espaços on fk_espacos = pk_espacos inner join enunciado on pk_enunciado = fk_enunciado
                    WHERE  
                    $espacos
                    $questao
                    $secretarias
    ...
    //echo $query_pesquisas;
    $dadosPesquisas = mysqli_query($conexao, $query_pesquisas) or die("Error in query_pesquisas: $query_pesquisas." . mysqli_error($conexao));
    //print_r($query_pesquisas);
}

#Lista de secretarias
$querySecretarias = 'SELECT DISTINCT (secretaria) FROM reservas INNER JOIN eventos ON fk_eventos = pk_eventos';
$dados_secretarias = mysqli_query($conexao, $querySecretarias) or die("Error in query_pesquisas: $querySecretarias." . mysqli_error($conexao));

#Lista de questões
$queryQuestao = 'SELECT DISTINCT * FROM enunciado';
$dados_questao = mysqli_query($conexao, $queryQuestao) or die("Error in query_pesquisas: $queryQuestao." . mysqli_error($conexao));

#Lista de espaços
$queryEspacos = 'SELECT esp.pk_espacos, esp.nome, esp.localizacao, esp.capacidade, 
...
$dados_espacos = mysqli_query($conexao, $queryEspacos) or die("Error in query_pesquisas: $queryEspacos." . mysqli_error($conexao));
...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main>
                ...
                <div class="container-fluid">
                    <form action="relatoriosSatisfacao.php" method="post">
                        <!-- Page Heading -->
                        <div class="mb-4 mt-4">
                            <h1 class="h4 text-primary">Relatórios Pesquisa de Satisfação</h1>
                        </div>
                            ...
                            <div class="container-fluid">
                                <div class="form-row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control py-2 valor" id="inputData" name="inputData" max=<?= $datainicial; ?> <?= !empty($checkDiaSemana) ? $checkDiaSemana : ""; ?> type="date" onchange="controlStart();" value="<?= isset($_REQUEST['requestSatisfacao']) ? $_REQUEST["inputData"] : ""; ?>">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="small mb-1" id="labelData" for="inputDataFim">Data Fim:</label>
                                            <input class="form-control py-2 valor" id="inputDataFim" name="inputDataFim" type="date" <?= !empty($checkDiaSemana) ? $checkDiaSemana : ""; ?> onchange="controlStart();" value="<?= isset($_REQUEST['requestSatisfacao']) ? $_REQUEST["inputDataFim"] : ""; ?>">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="col-md-6">
                                        ...
                                        <select selected id="inputDiaSemana" name="inputDiaSemana" class="form-group form-control" id="exampleFormControlSelect1" onchange="desabilitaDia(); changeFunc();" value="<?= isset($_REQUEST['requestSatisfacao']) ? $_REQUEST["inputDiaSemana"] : ""; ?>">
                                    ...
                                    <div class="col-md-6">
                                        <label for="exampleFormControlSelect1">Questões</label>
                                        <select id="inputQuestao" name="inputQuestao" class="form-control form-group">
                                            <option value="">Todas</option>
                                            <?php
                                            while ($linha = mysqli_fetch_assoc($dados_questao)) {
                                                $selected = $_REQUEST["inputQuestao"] == $linha["pk_enunciado"] ? "selected" : "";
                                                echo '<option ' . $selected . ' value="' . $linha['pk_enunciado'] . '">' . $linha['enunciado'] . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div style="margin-left:auto;">
                                        <input type="submit" class="btn btn-primary" name="requestSatisfacao" value="Gerar" onclick="gerarSatisfacao();">
                                    </div>
                                </div>
                    </form>
                    ...
                    <div class="card shadow mb-4 mt-3">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Lista de Pesquisas Geradas</h6>
                        </div>
                        <div class="card-body">
                                    ...
                                    <table id="visul_usuario" class="table table-bordered" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            ...
                                            if (!empty($dadosPesquisas)) {
                                                if (mysqli_num_rows($dadosPesquisas) > 0) {
                                                    while ($linha = mysqli_fetch_assoc($dadosPesquisas)) {
                                            ?>
    ...
    <script>
        ...
        $(document).ready(function utf8_decode() {
